    ...
    return (
        <div className="border-y border-slate-200 bg-slate-50 overflow-hidden relative" aria-hidden="true">
            ...
            <div className="absolute top-3 right-3 p-2 bg-white/60 backdrop-blur-sm rounded-md border border-slate-900/10 shadow">
                {/* Fix: Remove invalid 'title' prop from LayoutIcon and RobotIcon calls */}
                {isApp 
                    ? <LayoutIcon className="w-6 h-6 text-slate-600" /> 
                    : isCode ? <div className="font-mono text-xs font-bold text-slate-600 px-1">&lt;/&gt;</div> : <RobotIcon className="w-6 h-6 text-slate-600" />
                }
            </div>
        </div>
    );
